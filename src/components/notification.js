...
const createOrderConfirmation = (myArray) => {
    ...
    notificationWrapper.classList.add('notification__wrapper');
    ...
    confirmationList.classList.add('confirmation-list');
    myArray.forEach(item => {
        const itemDiv = createCartItemCard(item.name, item.price, item.count, item.thumbnail, true);
        confirmationList.appendChild(itemDiv);
    });

    const totalWrapper = document.createElement('div');
    totalWrapper.classList.add('total-wrapper');
    totalWrapper.innerHTML = `<p>Order Total</p> <p class="total-price">$${calculateCartTotal(myArray)}</p>`;
    notificationContent.append(confirmationList, totalWrapper);
