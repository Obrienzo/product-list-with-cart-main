...
import createCartItemCard from './cartItem';
...
function createCartObject(name, price, count) {
  const cartItem = {
    name,
    price,
    count
...
function parentEventHandler(ev) {
  const item = createCartObject(ev.detail.name, ev.detail.price, ev.detail.value);
...
function calculateCartTotal(myArray) {
